...
export default class GraphSimplifier {
	...
	runSimplifyGraph(joinLoops: boolean = true): Result {
		...
		if (startIndex !== -1) {
			...
			let edgesFound = 0
			const visitBoundaryCluster = (i: number, from: number, growth: Map<number, NodeVisit>, visits: number[], nextVisits: Denque<NextVisit>) => {
				let count = 0
		...
		for (const [i, entry] of this.entries.entries()) {
			for (const [j, path] of entry.conns.entries()) {
				if (path.isBoundary && !path.loopVisited) {
					const loop = buildLoop([i, j], this.entries, path => path.inSimplified)
					...
					for (const [i, current] of loop.entries()) {
						...
						if (prevPath.isBoundary === currentPath.isBoundary && nodeConnections[current] === 2 && isZero(area(prevEntry.coord, currentEntry.coord, this.entries[next]!.coord))) {
							keepNode[i] = false
						} else {
