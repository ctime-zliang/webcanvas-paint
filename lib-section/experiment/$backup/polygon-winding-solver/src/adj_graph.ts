import { contains, direction, edgeBox, equals, forEachPair, intersect, norm, Edge, Point } from './utils'
...
export default class AdjacencyGraph {
	...
	private addEdge(start: number, end: number, ignore: boolean = false) {
		...
		if (start != end) {
			...
			for (const [i, existing] of this.entries.entries()) {
				if (i === start || i === end || contains([startEntry.coord, endEntry.coord], existing.coord)) {
					checked[i] = true
					intermediates.push({
						index: i,
			...
			if (newIntermediates.length > 0) {
				const edges = newIntermediates.map(segment => {
					const edge: Edge = [this.entries[segment[0]!.index]!.coord, this.entries[segment[1]!.index]!.coord]
					return {
						edge,
						box: edgeBox(edge),
					}
						...
						for (const j of iEntry.conns.keys()) {
							if (j > i && j < checked.length && !checked[j]) {
								...
								for (const [segIndex, segment] of newIntermediates.entries()) {
									...
									if (incidentBox[0].x < edgeData.box[1].x && incidentBox[1].x > edgeData.box[0].x && incidentBox[0].y < edgeData.box[1].y && incidentBox[1].y > edgeData.box[0].y) {
