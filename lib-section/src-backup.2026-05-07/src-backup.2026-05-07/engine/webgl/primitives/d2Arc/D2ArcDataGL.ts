...
import { ECanvasD2LineCap } from '../../../config/PrimitiveProfile'
...
export class D2ArcDataGL {
	public static ITEM_SIZE: number = 21
	public static STRIDE: number = 21 * FLOAT_32_ARRAY_BYTESIZE
	public static createArrayData(primitiveItemValueData: TElementD2ArcJSONData, layerAlpha: number = 1.0): Float32Array {
		const { startRadian, endRadian, sweep } = primitiveItemValueData
		let sng: number = startRadian % (Math.PI * 2)
		let eng: number = endRadian % (Math.PI * 2)
		...
		const typedArray: Array<number> = [
			...
			primitiveItemValueData.lineCap === ECanvasD2LineCap.ROUND ? 1.0 : 0.0, // 10
