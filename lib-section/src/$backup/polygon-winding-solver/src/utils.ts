export const forEachPair = <T>(iter: IterableIterator<T>, action: (a: T, b: T) => void, loopOver: boolean = false) => {
...
export const equals = (p: Point, q: Point) => isZero(p.x - q.x) && isZero(p.y - q.y)

// 1-norm is the fastest norm to compute
export const norm = (p: Point, q: Point) => Math.abs(p.x - q.x) + Math.abs(p.y - q.y)

// dot product between (a - b) and (c - b)
export const dot = (a: Point, b: Point, c: Point) => (a.x - b.x) * (c.x - b.x) + (a.y - b.y) * (c.y - b.y)

// cross product between (a - b) and (c - b)
export const area = (a: Point, b: Point, c: Point) => (a.x - b.x) * (c.y - b.y) - (a.y - b.y) * (c.x - b.x)
...
export const direction = (edge: Edge) => Math.atan2(edge[1].y - edge[0].y, edge[1].x - edge[0].x)

export const contains = (edge: Edge, target: Point) => isZero(area(edge[0], target, edge[1])) && dot(edge[0], target, edge[1]) < 0
...
export const triangleBox = (trig: Triangle): [Point, Point] => {
	const x0 = trig[0].x < trig[1].x ? (trig[0].x < trig[2].x ? trig[0].x : trig[2].x) : trig[1].x < trig[2].x ? trig[1].x : trig[2].x
	const x1 = trig[0].x > trig[1].x ? (trig[0].x > trig[2].x ? trig[0].x : trig[2].x) : trig[1].x > trig[2].x ? trig[1].x : trig[2].x
	const y0 = trig[0].y < trig[1].y ? (trig[0].y < trig[2].y ? trig[0].y : trig[2].y) : trig[1].y < trig[2].y ? trig[1].y : trig[2].y
	const y1 = trig[0].y > trig[1].y ? (trig[0].y > trig[2].y ? trig[0].y : trig[2].y) : trig[1].y > trig[2].y ? trig[1].y : trig[2].y
	return [
		{ x: x0 - THRESHOLD, y: y0 - THRESHOLD },
		{ x: x1 + THRESHOLD, y: y1 + THRESHOLD },
	]
...
export class HitTestTriangle {
	...
	contains(point: Point) {
		if (point.x > this.box[0].x && point.x < this.box[1].x && point.y > this.box[0].y && point.y < this.box[1].y) {
