...
const triangulate = (paths: Point[][], triangulator: Triangulator = new EarcutTriangulator()): Result => {
	...
	if (triangulator.type === TriangulatorType.planeGraph) {
		...
		const edges = result.edges.map<EdgeIndex>(edge => [translator.translate(edge[0]), translator.translate(edge[1])])
	...
	return {
		trigs,
		edges: result.edges.map(edge => [graph.entries[edge[0]]!.coord, graph.entries[edge[1]]!.coord]),
		conns: result.conns.map(edge => [graph.entries[edge[0]]!.coord, graph.entries[edge[1]]!.coord]),
	}
}
