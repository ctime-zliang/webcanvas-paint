...
export const buildLoop = (start: EdgeIndex, entries: PointEntry[], filter: ((path: PathEntry) => boolean) | undefined = undefined) => {
	...
	while (true) {
		...
		let index = 0
		while (index < currentEntry.directions.length && currentEntry.directions[index] !== loop[loop.length - 2]) {
			index += 1
		}
		index = (index + 1) % currentEntry.directions.length
		while (filter !== undefined && !filter(currentEntry.conns.get(currentEntry.directions[index]!)!)) {
			index = (index + 1) % currentEntry.directions.length
		}
				...
				if (path.loop === -1) {
					...
					for (const [i, index] of loop.entries()) {
						if (index == leftNode) {
							...
							if (leftNode === -1 || entry.coord.x < leftCoord.x || (entry.coord.x === leftCoord.x && entry.coord.y < leftCoord.y)) {
								leftIndices = [i]
								leftNode = index
					...
					for (const leftIndex of leftIndices) {
						...
						if (loop[prevIndex] === loop[nextIndex] || area(prevEntry.coord, leftCoord, nextEntry.coord) > 0) {
							isExterior = true
							break
						}
					}
