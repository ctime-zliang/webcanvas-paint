...
const pushSequence = <T>(list: Array<DoublyLinkedNode<T>>, sequence: Array<T>): EdgeIndex => {
...
const join = <T>(list: Array<DoublyLinkedNode<T>>, first: number, second: number) => {
	list[first]!.next = second
	list[second]!.prev = first
}

const remove = <T>(list: Array<DoublyLinkedNode<T>>, node: DoublyLinkedNode<T>) => {
	join(list, node.prev, node.next)
}

const moveToBefore = <T>(list: Array<DoublyLinkedNode<T>>, source: number, dest: number) => {
	...
	triangulate(points: Point[], loop: number[]) {
		...
		while (actions[ACTION_CUT]!.next !== ACTION_NONE) {
			...
			if (loopNodes[node]!.next === node) {
				...
				const a = area(points[prevPoint]!, points[currentPoint]!, points[nextPoint]!)
				let cut = false
				if (isZero(a)) {
					if (dot(points[prevPoint]!, points[currentPoint]!, points[nextPoint]!) > 0) {
						...
						const pArea = area(points[loopNodes[loopNodes[nodeEntry.prev]!.prev]!.data]!, points[prevPoint]!, points[currentPoint]!)
						const nArea = area(points[currentPoint]!, points[nextPoint]!, points[loopNodes[loopNodes[nodeEntry.next]!.next]!.data]!)
						cut = pArea > 0 && nArea > 0
					}
				} else if (a < 0) {
					const triangle: Triangle = [points[prevPoint]!, points[currentPoint]!, points[nextPoint]!]
					...
					while (loopNodes[current]!.next !== nodeEntry.prev) {
						...
						if (point !== prevPoint && point !== currentPoint && point !== nextPoint && testTriangle.contains(points[point]!)) {
							cut = false
						}
					}
