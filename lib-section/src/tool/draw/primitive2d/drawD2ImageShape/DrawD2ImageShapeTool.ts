...
import { DrawD2ImageShape } from './DrawD2ImageShape'

export class DrawD2ImageShapeTool extends DrawD2ShapeTool {
	private _drawShapeHandler: DrawD2ImageShape
	...
	public mouseMoveHandler(inputInfo: InputInfo): void {
		...
		const diffX: number = inputInfo.moveRealScenePhysicsX - this.lastMoveRealScenePhysicsX
		const diffY: number = inputInfo.moveRealScenePhysicsY - this.lastMoveRealScenePhysicsY
	...
	public mouseLeftUpHandler(inputInfo: InputInfo): void {
		...
		if (!this.isDrawing) {
			...
			if (this._drawShapeHandler && this._drawShapeHandler.isShapeInsatncesContentReady()) {
				...
				const items: Array<D2ImageShape> = this._drawShapeHandler.completeDraw()
