...
export class D2PointView extends D2ShapeElementViewBase {
	...
	public modify(shapeObjectItem: D2PointShape): void {
		...
		this.status = status
		const shapeObjectItemJson: TElement2DPointJSONViewData = shapeObjectItem.toJSON()
		if (this.killed) {
			this.delete()
	...
	public normalview(shapeObjectItem: D2PointShape): void {
		if (this._maskPrimitive) {
			this._maskPrimitive.delete()
			this._maskPrimitive = undefined!
		}
