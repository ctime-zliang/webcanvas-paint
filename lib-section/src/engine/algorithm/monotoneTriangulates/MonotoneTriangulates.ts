...
/**
 * MonotoneTriangulates - 单调多边形三角剖分(扫描线算法)
 *
 * 单调多边形定义:
 * 		- 相对于某一条直线(称为单调方向), 多边形的外轮廓与该直线的交点数量不超过两个
 *
 * 本实现中单调方向为 X 轴方向
 *
 * 算法概述 - 平面扫描三角剖分:
 * 		- 构建事件队列: 将点和边转化为"点事件"和"边事件" (START/END)
 * 		- 按 X 坐标排序事件(从左到右扫描)
 * 		- 维护一组"部分凸包" (PartialHull), 代表当前扫描线左侧未闭合的三角剖分区域
 * 		- 处理三种事件:
 *    		- POINT: 将新点添加到受影响的凸包中, 生成三角形
 *    		- START: 在事件位置分割凸包
 *    		- END: 在事件位置合并相邻凸包
 *
 *
 ...
 * 				[(0, 1), (1, 2), (2, 0)]
 * 			事件排序后(按X坐标):
 * 				- x = 0: POINT(0, 0), START 边 (0 → 4, 2), START 边 (0 → 2, 4)
 * 				- x = 2: POINT(2, 4), END 边 (0 → 2, 4), START 边 (2, 4 → 4, 2)
 * 				- x = 4: POINT(4, 2), END 边 (0 → 4, 2), END 边 (2, 4 → 4, 2)
 * 			最终输出一个三角形
 * 				[0, 1, 2]
 */
