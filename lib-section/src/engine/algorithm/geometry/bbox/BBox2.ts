...
export class BBox2 {
	...
	public isContainsValue(x: number, y: number): boolean {
	...
	public getIntersection(bbox2: BBox2): BBox2 {
		let [minX, maxX]: [number, number] = [Math.max(this.minX, bbox2.minX), Math.min(this.maxX, bbox2.maxX)]
		...
		let [minY, maxY]: [number, number] = [Math.max(this.minY, bbox2.minY), Math.min(this.maxY, bbox2.maxY)]
