...
import { D2ShapeElementViewBase } from '../views/shapes/primitive2d/elementBase/D2ShapeElementViewBase'

export class ShapeViewManager extends BaseManager<D2ShapeElementViewBase> {
	...
	public handleModify(scene: Scene, elements: Set<D2ElementShapeItemBase>): void {
	...
	public modifyItem(elementShapeObject: D2ElementShapeItemBase): void {
		const { elementItemId } = elementShapeObject
		const elementType: ED2ElementType = elementShapeObject.getType()
		let elementItem: D2ShapeElementViewBase = this.items.get(elementItemId) as D2ShapeElementViewBase
		if (!elementItem) {
			let newElementItem: D2ShapeElementViewBase = null!
			switch (elementType) {
	...
	public deleteItem(elementItemId: string): void {
		const elementItem: D2ShapeElementViewBase = this.items.get(elementItemId)!
		if (!elementItem) {
			return
		}
		elementItem.delete()
