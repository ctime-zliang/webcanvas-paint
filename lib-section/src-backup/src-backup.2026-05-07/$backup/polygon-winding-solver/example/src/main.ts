...
import triangulate, { Edge, EdgeIndex, LoopTriangulator, PlaneGraphTriangulator, Point, Triangle, Triangulator, TriangulatorType } from '../..'
...
class EarcutTriangulator implements LoopTriangulator {
	...
	triangulate(points: Point[], loop: number[]) {
		...
		for (let i = 0; i + 2 < verts.length; i += 3) {
			trigs.push([vertex(verts[i]!), vertex(verts[i + 1]!), vertex(verts[i + 2]!)])
		}
		return trigs
	}
}
...
class CDT2DTriangulator implements PlaneGraphTriangulator {
	...
	triangulate(points: Point[], edges: EdgeIndex[]) {
		...
		return trigs.map<Triangle>(t => [points[t[0]]!, points[t[1]]!, points[t[2]]!])
	}
}
...
const drawPoint = (context: CanvasRenderingContext2D, point: Point, radius: number) => {
...
const draw = () => {
	...
	const computed = triangulator === undefined ? triangulate(paths) : triangulate(paths, triangulator)
