...
export class SelectManager extends BaseManager<D2ElementShapeItemBase> {
	 ...
	 * 获取所有被选中的图元
	 */
	public getAllSelectItems(): Array<D2ElementShapeItemBase> {
		const selects: Array<D2ElementShapeItemBase> = []
	...
	public clearSelectItemById(elementItemId: string): void {
		if (this.items.has(elementItemId)) {
			const elementItem: D2ElementShapeItemBase = this.items.get(elementItemId)!
	...
	public mouseLeftDownHandler(inputInfo: InputInfo): void {
		...
		if (Constant.systemConfig.interactive.enableCanvasSelection) {
			...
			const selectResults: Set<D2ElementShapeItemBase> = this.pointSelect(inputInfo)
			if (clickSelect) {
	...
	public mouseLeftUpHandler(inputInfo: InputInfo): void {
		...
		if (Constant.systemConfig.interactive.enableCanvasSelection) {
			if (this._isBoxSelection) {
				const selectResults: Set<D2ElementShapeItemBase> = this.boxSelect(inputInfo)
				this.setSelectStatus(selectResults)
			}
		}
	 ...
	 * 		设置图元的选中样式
	 */
	private setSelectStatus(selectedItems: Set<D2ElementShapeItemBase>): void {
		for (let [key, item] of this.items) {
			item.setUnSelect()
		}
		this.items.clear()
