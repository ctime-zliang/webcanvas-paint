...
export class D2ArcToolkit {
	...
	public static updateArcParamByNewStartPoint(
		...
		const centerPoint: Vector2 = mid.add(l.mul(radius - distance))
		const [OS, OE]: [Vector2, Vector2] = [newStartPoint2.sub(centerPoint).normalize(), oldEndPoint.sub(centerPoint).normalize()]
	...
	public static updateArcParamByNewEndPoint(
		...
		const centerPoint: Vector2 = mid.add(l.mul(radius - distance))
		const [OS, OE]: [Vector2, Vector2] = [oldStartPoint.sub(centerPoint).normalize(), newEndPoint.sub(centerPoint).normalize()]
		result.startRadian = Math.atan2(OS.y, OS.x)
		result.endRadian = Math.atan2(OE.y, OE.x)
		result.centerPoint = centerPoint
		return result
