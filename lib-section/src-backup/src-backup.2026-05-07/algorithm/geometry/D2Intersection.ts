...
export class D2Intersection {
	...
	public static getIntersectionsOfLineAndArc(line: Line, arc: Arc): { count: number; points: Array<Vector2> } {
		if (arc.rx === arc.ry) {
			...
			const [cx, cy, r, r2]: [number, number, number, number] = [arc.centerPoint.x, arc.centerPoint.y, arc.rx, arc.rx * arc.rx]
	...
	public static getIntersectionsOfArcs(arcA: Arc, arcB: Arc): { count: number; points: Array<Vector2> } {
		if (arcA.rx === arcA.ry && arcB.rx === arcB.ry) {
			...
			const [r1, r2]: [number, number] = [arcA.rx, arcB.rx]
			const [r21, r22]: [number, number] = [arcA.rx * arcA.rx, arcB.rx * arcB.rx]
