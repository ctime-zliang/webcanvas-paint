...
import { D2Distance } from './D2Distance'
import { D2PolylineToolkit } from './D2PolylineToolkit'
	...
	private getRelation(): {
		...
		for (let pt1 of this._pl1.primitives) {
			for (let pt2 of this._pl2.primitives) {
				if (pt1 instanceof Line && DoubleKit.eq(pt1.startPoint.distanceSquare(pt1.endPoint), 0)) {
					continue
				}
				if (pt2 instanceof Line && DoubleKit.eq(pt2.startPoint.distanceSquare(pt2.endPoint), 0)) {
					continue
				}
				const dt: D2Distance = new D2Distance(pt1, pt2)
		...
		if (result.relation !== ED2ElementGeometryRelation.Intersect) {
			if (result.pt1 && D2PolylineToolkit.isPolylineClosed(this._pl2) && this.pointInPolyline(result.pt1.startPoint, this._pl2)) {
				result.relation = ED2ElementGeometryRelation.BContainA
				if (this._setDistZeroWhenContain) {
					result.distance = 0
				}
			} else if (result.pt2 && D2PolylineToolkit.isPolylineClosed(this._pl1) && this.pointInPolyline(result.pt2.startPoint, this._pl1)) {
